...
public class HeapSort2 {
	...
	public static void build_max_heap2(int a[]) {
		for (int i = a.length/2; i >= 0; i--) {
			shiftDown2(a, i, a.length);
		}
	}
	
	public static void shiftDown2(int a[], int index, int length) {
		int maxIndex = index;
	...
	public static void shiftDown(int a[], int index, int length) {
		int maxIndex = index;
		// check left
		int l = left(index);
		if (l < length && a[l] > a[index]) {
		...
		// which ever is bigger, shift accordingly
		if (maxIndex != index) {
			// swap max index
			int temp = a[maxIndex];
			a[maxIndex] = a[index];
			a[index] = temp;
			shiftDown(a, maxIndex, length);
		}
	}
